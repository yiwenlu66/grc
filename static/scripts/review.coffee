...
root.get_blanks_random = (question) ->
    ...
    for i in [0..phrases.length-1]
        phrase=phrases[i]
        is_blank=(if i is phrases.length-1 and num_blanks is 0 then true else if i is phrases.length-1 and num_blanks is phrases.length-2 then false else Math.random()<0.4)
